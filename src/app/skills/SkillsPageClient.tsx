...
export default function SkillsPageClient({ skillGroups }: SkillsPageClientProps) {
  const [activeGroup, setActiveGroup] = useState<string | null>(null);
  const [activeSkill, setActiveSkill] = useState<string | null>(null);

  const filteredProjects =
    activeSkill && activeGroup
      ? skillGroups
          .find((g) => g.id === activeGroup)
          ?.skills.find((s) => s.name === activeSkill)?.projects ?? []
      : [];
        ...
        <div className="max-w-3xl mb-16 md:mb-20">
          ...
          <FadeIn delay={0.2}>
            <p className="mt-6 text-base md:text-lg text-[var(--color-text-secondary)] leading-relaxed max-w-2xl">
              Skills proven through projects. Select a skill to see the work that demonstrates it.
            </p>
          </FadeIn>
        </div>
                ...
                >
                  <h3
                    className={`text-lg font-medium transition-colors duration-300 ${
                      activeGroup === group.id ? "text-[var(--color-accent)]" : "text-[var(--color-text-primary)]"
                    }`}
                  >
                    {group.name}
                  </h3>
                  <p className="mt-2 text-sm text-[var(--color-text-secondary)]">{group.description}</p>
              ...
              <div className="flex flex-wrap gap-3">
                {skillGroups
                  ...
                  ?.skills.map((skill) => (
                    ...
                    >
                      {skill.name}
                      <span className="ml-2 text-xs text-[var(--color-text-muted)]">{skill.projects.length}</span>
                    </button>
                  ))}
              </div>
            </div>
